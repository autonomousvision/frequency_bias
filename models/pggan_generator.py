"""Adapted from https://github.com/genforce/genforce/blob/master/models/pggan_generator.py
Added different upsampling modes and padding modes"""
# python3.7
...
from .utils import UPSAMPLE_FNS
...
class PGGANGenerator(nn.Module):
    ...
    (2) z_dim: The dimension of the latent space, Z. (default: 512)
    ...
    def __init__(self,
        ...
        self.divide_channels_by = divide_channels_by
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            is_last = res_log2 == self.final_res_log2
            ch_multiplier = 4 if (upsampling_mode == 'shuffle' and not is_last) else 1  # pixel shuffle needs 4x channel dim
            ...
            if res == self.init_res:
                self.add_module(
                    ...
                    ConvBlock(in_channels=self.z_space_dim + self.label_size,
                              ...
                              padding_mode='zeros',             # always start with zero padding
            ...
            self.add_module(
                ...
                ConvBlock(in_channels=self.get_nf(res),
                          out_channels=self.get_nf(res) * ch_multiplier,
                          use_wscale=self.use_wscale,
                          padding_mode=padding_mode))
        ...
        self.upsample = UpsamplingLayer(mode='nearest')         # for fade-in always use nearest neighbor interpolation
        self.final_activate = nn.Tanh() if self.final_tanh else nn.Identity()

    def get_nf(self, res):
        """Gets number of feature maps according to current resolution."""
        return min(self.fmaps_base // res, self.fmaps_max) // self.divide_channels_by
...
class UpsamplingLayer(nn.Module):
    ...
    Basically, this layer can be used to upsample feature maps with bilinear or nearest
    neighbor interpolation or zero insertion or pixel shuffle.
    """
    ...
    def __init__(self, scale_factor=2, mode='nearest'):
        ...
        # Added
        assert mode in UPSAMPLE_FNS
        self.mode = mode
        self.up = UPSAMPLE_FNS[mode]

    def forward(self, x):
        if self.scale_factor <= 1:
            return x
        return self.up(x, scale_factor=self.scale_factor)

    def extra_repr(self):
        s = ('upsampling_mode={mode}')
...
class ConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if upsample and not fused_scale:
            self.upsample = UpsamplingLayer(mode=upsampling_mode)
        else:
            self.upsample = nn.Identity()

        if upsample and fused_scale:
            raise RuntimeWarning('Custom upsampling and padding operations not implemented with fused_scale.')
    ...
    def forward(self, x):
        ...
        if self.use_conv2d_transpose:
            ...
            if self.padding_mode != 'zeros':
                _reversed_padding_repeated_twice = torch.nn.modules.utils._reverse_repeat_tuple(torch.nn.modules.utils._pair(self.padding), 2)
    ...
    def extra_repr(self):
        out_channels, in_channels, kernel_size = self.weight.shape[:3]
        s = (f'Conv2D({in_channels}, {out_channels}, kernel_size={kernel_size}'
             ', stride={stride}, use_conv2d_transpose={use_conv2d_transpose}')
        if self.padding != 0:
            s += ', padding={padding}'
