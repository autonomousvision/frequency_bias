"""Adapted from https://github.com/genforce/genforce/blob/master/models/pggan_discriminator.py
Added different downsampling modes and padding modes"""
# python3.7
...
from .utils import DOWNSAMPLE_FNS
...
class PGGANDiscriminator(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample = DownsamplingLayer(mode='blurpool')         # for fade-in always use blurpool downsampling

    def get_nf(self, res):
        """Gets number of feature maps according to current resolution."""
        return min(self.fmaps_base // res, self.fmaps_max) // self.divide_channels_by

    def forward(self, image, c=None, lod=None, **_unused_kwargs):
...
class DownsamplingLayer(nn.Module):
    ...
    def __init__(self, scale_factor=2, mode='avg'):
        super().__init__()
        self.scale_factor = scale_factor
        # Added
        assert mode in DOWNSAMPLE_FNS
        self.mode = mode
        self.down = DOWNSAMPLE_FNS[mode]

    def forward(self, x):
        if self.scale_factor <= 1:
            return x
        return self.down(x, scale_factor=self.scale_factor)

    def extra_repr(self):
        s = ('downsampling_mode={mode}')
...
class ConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if downsample and not fused_scale:
            self.downsample = DownsamplingLayer(mode=downsampling_mode)
        else:
            self.downsample = nn.Identity()

        if downsample and fused_scale:
            raise RuntimeWarning('Custom downsampling and padding operations not implemented with fused_scale.')
    ...
    def forward(self, x):
        ...
        if self.padding_mode != 'zeros':
            _reversed_padding_repeated_twice = torch.nn.modules.utils._reverse_repeat_tuple(
                torch.nn.modules.utils._pair(self.padding), 2)
    ...
    def extra_repr(self):
        ...
        s = (f'Conv2D({in_channels}, {out_channels}, kernel_size={kernel_size}'
             ', stride={stride}, use_stride={use_stride}')
        if self.padding != 0:
            s += ', padding={padding}'
